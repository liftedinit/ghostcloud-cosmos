...
func TestMsgCreateDeployment_ValidateBasic(t *testing.T) {
	tests := []struct {
		...
		{
			name: "name too long",
			msg:  types.MsgCreateDeploymentRequest{Meta: &types.Meta{Creator: sample.AccAddress(), Name: strings.Repeat("a", 13)}},
			err:  sdkerrors.ErrInvalidRequest,
		},
		{
			name: "invalid domain",
		...
		},
		{
			...
			msg:  types.MsgCreateDeploymentRequest{Meta: &types.Meta{Creator: sample.AccAddress(), Name: "name", Description: strings.Repeat("a", 513)}},
			err:  sdkerrors.ErrInvalidRequest,
		},
		{
			name: "empty request",
